...
const fromEmail = process.env.RESEND_FROM_EMAIL || 'OneF1 <onboarding@resend.dev>';
...
export async function sendVerificationEmail(email: string, token: string) {
  ...
  try {
    const data = await resend.emails.send({
      from: fromEmail,
      to: [email],
      subject: 'Verify your email address - OneF1',
      html: `
        <div>
          <h2>Welcome to OneF1!</h2>
...
export async function sendPasswordResetEmail(email: string, token: string) {
  ...
  try {
    const data = await resend.emails.send({
      from: fromEmail,
      to: [email],
      subject: 'Reset your password - OneF1',
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
          <h2>Password Reset Request</h2>
          <p>We received a request to reset the password for your OneF1 account.</p>
